...
func (c *Conn) ReadData() (data []byte, err error) {
	...
	// 不支持数据分片
	if !final {
		log.Println("Received fragmented frame, not support")
	...
	if frameType == CloseMessage {
		c.conn.Close()
		log.Println("Received closed message, connection will be closed")
		return nil, errors.New("received closed message")
	}
